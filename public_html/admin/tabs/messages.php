...
// Get all users for the recipient list
...
async function loadMessages() {
    ...
    try {
        const response = await fetch(`/admin/api.php?action=get_messages&user_id=${currentRecipientId}`);
        ...
        if (result.success) {
            ...
            if (result.messages.length === 0) {
                ...
                result.messages.forEach(msg => {
                    ...
                    messageDiv.className = 'flex items-start gap-2 ' + (msg.is_sent ? 'justify-end' : 'justify-start');
                    ...
                    deleteBtn.className = 'text-gray-500 hover:text-red-400 transition opacity-0 group-hover:opacity-100';
                    deleteBtn.onclick = () => deleteMessage(msg.id);
                    deleteBtn.title = 'Delete message';
                    
                    const wrapper = document.createElement('div');
                    wrapper.className = 'message-wrapper group flex items-start gap-2 ' + (msg.is_sent ? 'flex-row-reverse sent' : 'flex-row');
...
async function sendMessage(event) {
    ...
    console.log('Sending message to user:', currentRecipientId);
