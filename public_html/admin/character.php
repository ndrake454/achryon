...
<div>
    <label class="block text-gray-300 mb-2 text-sm font-medium">Spellcasting Ability</label>
    <select 
        onchange="updateStat('spellcasting_ability', this.value)"
        class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-primary">
        <option value="">None</option>
        <option value="INT" <?php echo ($char['spellcasting_ability'] ?? '') === 'INT' ? 'selected' : ''; ?>>Intelligence</option>
        <option value="WIS" <?php echo ($char['spellcasting_ability'] ?? '') === 'WIS' ? 'selected' : ''; ?>>Wisdom</option>
        <option value="CHA" <?php echo ($char['spellcasting_ability'] ?? '') === 'CHA' ? 'selected' : ''; ?>>Charisma</option>
    </select>
</div>
...
async function updateStat(stat, value) {
    console.log('updateStat called:', stat, value); // Debug log
    ...
    try {
        ...
        const result = await response.json();
        console.log('updateStat result:', result); // Debug log
